...
/*
 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE:
    ...
    {
       // TODO: Setup renderer state: alpha-blending enabled, no face culling, no depth testing, scissor enabled
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              ...
              (1.30) - added texture identifier in ImDrawCmd passed to your renderer function (we can now renderer images). make sure to set io.Fonts->TexID.
 ...
 A: Short explanation:
    ...
    - Dear ImGui's job is to create "meshes", defined in a renderer-agnostic format made of draw commands and vertices.
      ...
      to renderer them is generally fairly short (a few dozen lines). In the examples/ folder we provide functions for popular graphics API (OpenGL, DirectX, etc.).
 ...
 A: - You can control Dear ImGui with a gamepad. Read about navigation in "Using gamepad/keyboard navigation controls".
      (short version: maps gamepad inputs into the io.NavInputs[] array + set io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad)
    ...
    - You may also use a third party solution such as Remote ImGui (https://github.com/JordiRos/remoteimgui) which sends
      the vertices to renderer over the local network, allowing you to use Dear ImGui even on a screen-less machine.
 ...
 A: You are probably mishandling the clipping rectangles in your renderer function.
...
// When using this function it is sane to ensure that float are perfectly rounded to integer values, to that e.g. (int)(max.x-min.x) in user's renderer produce correct result.
...
void ImGui::Render()
{
    ...
    // Gather ImDrawList to renderer (for each active window)
    ...
    // (Legacy) Call the Render callback function. The current prefer way is to let the user retrieve GetDrawData() and call the renderer function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Since 1.71, child window can renderer their decoration (bg color, border, scrollbars, etc.) within their parent to save a draw call.
...
static void ImGui::NavUpdate()
{
    ...
    // Set input source as Gamepad when buttons are pressed before we maps Keyboard (some features differs when used with Gamepad vs Keyboard)
