...
struct ImGuiContext
{
    ...
    // Render
    ImDrawData              DrawData;                           // Main ImDrawData instance to pass renderer information to the user
    ImDrawDataBuilder       DrawDataBuilder;
    float                   DimBgRatio;                         // 0.0..1.0 animation when fading in a dimming background (for modal window and CTRL+TAB list)
    ImDrawList              BackgroundDrawList;                 // First draw list to be rendered.
    ImDrawList              ForegroundDrawList;                 // Last draw list to be rendered. This is where we the renderer software mouse cursor (if io.MouseDrawCursor is set) and most debug overlays.
