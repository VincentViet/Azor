...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(), you likely don't need to call that yourself directly. If you don't need to renderer data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to renderer, better to not create any imgui windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function. (Obsolete: this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your renderer function yourself.)
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). this is what you have to renderer.
    ...
    // Inputs Utilities
    IMGUI_API int           GetKeyIndex(ImGuiKey imgui_key);                                    // maps ImGuiKey_* values into user's key index. == io.KeyMap[key]
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will renderer those for you
...
// Special Draw callback value to request renderer back-end to reset the graphics/renderer state.
...
// It is not done by default because they are many perfectly useful way of altering renderer state for imgui contents (e.g. changing shader/blending settings before an Image call).
#define ImDrawCallback_ResetRenderState     (ImDrawCallback)(-1)

// Typically, 1 command = 1 GPU draw call (unless command is a callback)
// Pre 1.71 back-ends will typically ignore the VtxOffset/IdxOffset fields. When 'io.BackendFlags & ImGuiBackendFlags_RendererHasVtxOffset'
// is enabled, those fields allow us to renderer meshes larger than 64K vertices while keeping 16-bits indices.
struct ImDrawCmd
{
...
struct ImDrawList
{
    ...
    // Advanced: Channels
    // - Use to split renderer into layers. By switching channels to can renderer out-of-order (e.g. submit foreground primitives before background primitives)
...
struct ImDrawData
{
    ...
    ImDrawList**    CmdLists;               // Array of ImDrawList* to renderer. The ImDrawList are owned by ImGuiContext and only pointed to from here.
    int             CmdListsCount;          // Number of ImDrawList* to renderer
    int             TotalIdxCount;          // For convenience, sum of all ImDrawList's IdxBuffer.Size
    int             TotalVtxCount;          // For convenience, sum of all ImDrawList's VtxBuffer.Size
    ImVec2          DisplayPos;             // Upper-left position of the viewport to renderer (== upper-left of the orthogonal projection matrix to use)
    ImVec2          DisplaySize;            // Size of the viewport to renderer (== io.DisplaySize for the main viewport) (DisplayPos + DisplaySize == lower-right of the orthogonal projection matrix to use)
...
struct ImFontAtlas
{
    ...
    // After calling Build(), you can query the rectangle position and renderer your pixels.
    // You can also request your rectangles to be mapped as font glyph (given a font + Unicode point), 
    // so you can renderer e.g. custom colorful icons and use them as regular glyphs.
    // Read misc/fonts/README.txt for more details about using colorful icons.
    struct CustomRect
    {
        unsigned int    ID;             // Input    // User ID. Use <0x10000 to maps into a font glyph, >=0x10000 for other/internal/custom texture data.
    ...
    IMGUI_API int       AddCustomRectFontGlyph(ImFont* font, ImWchar id, int width, int height, float advance_x, const ImVec2& offset = ImVec2(0,0));   // Id needs to be < 0x10000 to register a rectangle to maps into a specific font.
...
struct ImFont
{
    ...
    // Members: Hot ~36/48 bytes (for CalcTextSize + renderer loop)
